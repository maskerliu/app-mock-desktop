  ...
  <el-row id="app">
    <el-col style="width: 70px; height: 100vh;">
      <el-menu :default-active="curActivedNavMenuIdx" :collapse="false" style="width: 100%; height: 100%;">
        <el-menu-item index="0" style="padding-left: 20px;margin-top: 50px;" @click="updateShowQrCodeDialog(true)">
          <i class="iconfont icon-qrcode" style="font-size: 1.8rem;"></i>
        </el-menu-item>
        <el-tooltip effect="dark" :content="item.tip" placement="right" v-for="(item, idx) in navMenus" :key="idx">
    ...
    <el-col style="width: calc(100vw - 70px); height: calc(100vh - 10px); background: transparent;">
      <div class="header">
        ...
        <div class="header-right">
          <el-button type="info" plain @click="showLoginDialog = true">{{ username ? username : "登录" }}</el-button>
          <span class="el-icon-minus header-right-btn" style="color: #f39c12;" @click="onMinus()"></span>
          <span class="el-icon-full-screen header-right-btn" style="color: #2ecc71;" @click="onMaximize()"></span>
          <span class="el-icon-switch-button header-right-btn" style="color: #e74c3c;" @click="onClose()"></span>
        </div>
      </div>
    ...
    <el-dialog title="用户登录" :visible="showLoginDialog" @close="showLoginDialog = false">
      <el-input placeholder="请输入内容" v-model="username">
        <template slot="prepend">用户名: </template>
        <el-button slot="append" icon="iconfont icon-cloud-sync" @click="login"></el-button>
      </el-input>
    </el-dialog>
  </el-row>
</template>
